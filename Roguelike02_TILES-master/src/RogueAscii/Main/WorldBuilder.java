...
public class WorldBuilder {
	...
	{
		System.out.println("Method Name: createRegions()");
        regions = new int[width][height][depth];
        nextRegion = 1;
        System.out.println("Starting Region id:" +nextRegion);
        for (int z = 0; z < depth; z++)
        {
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                   if (tiles[x][y][z] != Tile.WALL && regions[x][y][z] == 0){
                	   System.out.println("Filling new Region, calling fillRegion() method");
                       int size = fillRegion(nextRegion++, x, y, z);
                       
              
                       if (size < 25)
                       {
                    	   System.out.println("Region size less than 25, calling removeRegion() method");
        ...
        System.out.println("Final number of Regions:" +nextRegion);
        return this;
	...
	private void removeRegion(int region, int z)
	{
	    for (int x = 0; x < width; x++)
	    {
	        for (int y = 0; y < height; y++)
	        {
	            ...
	            {
	            	System.out.println("Method Name: removeRegion( int "+region+ " int "+z+")");
	            	System.out.println("Current tile: "+ tiles[x][y][z]);
	                regions[x][y][z] = 0;
	                tiles[x][y][z] = Tile.WALL;
	                System.out.println("Changed to: "+ tiles[x][y][z]);
	            }
	        }
	    }
	...
	{
		System.out.println("Method Name: fillRegion( int region: "+region+ " int x: "+x+" int y: "+y+" int z: "+z+")");
        int size = 1;
        System.out.println("Initial region size: "+ size);
        ...
        System.out.println("Final region size: "+ size);
        return size;
    }
